...
def run_ifta():
    ...
    with ifta_tab1:
        ...
        with st.form("trip_form"):
            ...
            with col1:
                ...
                loaded_miles = st.number_input("Loaded Miles", min_value=0, step=1)
            with col2:
                destination = st.text_input("Destination (City, ST)")
                deadhead_miles = st.number_input("Deadhead Miles (empty)", min_value=0, step=1)
            
            total_miles = loaded_miles + deadhead_miles
            st.info(f"**Total Miles:** {total_miles}")
            
            submitted = st.form_submit_button("Save Trip")
            if submitted and (loaded_miles > 0 or deadhead_miles > 0):
                ...
                st.success(f"✅ Trip saved! ({total_miles} miles)")
            elif submitted:
                st.warning("Please enter at least some miles.")
        
        # Show existing trips
        if st.session_state.ifta_trips:
            st.subheader("📋 Trip History")
            st.dataframe(st.session_state.ifta_trips)
    ...
    with ifta_tab3:
        ...
        if not st.session_state.ifta_trips and not st.session_state.ifta_fuel:
            st.info("No trip or fuel data yet. Start by logging trips and fuel purchases.")
            return
        ...
        for trip in st.session_state.ifta_trips:
            ...
            origin_state = trip['origin'][-2:] if len(trip['origin']) >= 2 else 'TX'
            dest_state = trip['destination'][-2:] if len(trip['destination']) >= 2 else 'TX'
            
            half_miles = trip['total_miles'] / 2
            total_miles_by_state[origin_state] = total_miles_by_state.get(origin_state, 0) + half_miles
            total_miles_by_state[dest_state] = total_miles_by_state.get(dest_state, 0) + half_miles
        
        # Calculate total fuel by state
        total_fuel_by_state = {}
        total_gallons = 0
